...
export default function DashboardPage() {
  ...
  const fetchVolunteers = useCallback(async () => {
    ...
    try {
      ...
      setError("Could not load volunteers. Try refreshing.")
    } finally {
      setLoading(false)
    }
  }, [])
  ...
  return (
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8 space-y-8">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div>
          <h2 className="text-2xl font-extrabold text-gray-900">Volunteers</h2>
          <p className="text-sm text-gray-500 mt-0.5">All campaign volunteer registrations from the website</p>
        </div>
      ...
      <div className="bg-white rounded-2xl border border-slate-100 shadow-sm p-5 space-y-4">
        ...
        {(search || stateFilter || interestFilter) && (
          <button onClick={() => { setSearch(""); setStateFilter(""); setInterestFilter("") }}
      ...
      <div className="bg-white rounded-2xl border border-slate-100 shadow-sm overflow-hidden">
        {loading ? (
          ...
          <div className="text-center py-24 space-y-3">
            ...
            <button onClick={fetchVolunteers} className="text-xs text-red-600 underline">Try again</button>
          </div>
        ) : filtered.length === 0 ? (
          <div className="flex flex-col items-center justify-center py-24 gap-3 text-slate-400">
            <Users className="w-12 h-12 text-slate-200" />
            <p className="font-medium">{volunteers.length === 0 ? "No volunteers yet" : "No results found"}</p>
            <p className="text-xs">{volunteers.length === 0 ? "They will appear here once people sign up" : "Try adjusting your filters"}</p>
          </div>
        ) : (
          <>
            {/* Desktop */}
                ...
                <tbody className="divide-y divide-slate-50">
                  {filtered.map((v, idx) => (
                    <tr key={v.id} className="hover:bg-slate-50/80 transition group">
                      ...
                      <td className="px-4 py-3 font-semibold text-slate-800">{v.full_name}</td>
            ...
            <div className="md:hidden divide-y divide-slate-100">
              {filtered.map(v => (
                <div key={v.id} className="p-4 space-y-3 active:bg-slate-50" onClick={() => setSelected(v)}>
            ...
            <div className="px-6 py-3 border-t border-slate-100 bg-slate-50 text-xs text-slate-400">
              ...
              <span className="font-semibold text-slate-600">{volunteers.length}</span> volunteers
            </div>
          </>
        )}
      </div>
